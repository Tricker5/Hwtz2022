...
struct AllocMgr{
    public:
        ...
        Solutions solveDemands();
        bool solveOneCstmDm(vector<Customer*> &vec_cstm, const size_t dm_idx, Solutions &slts);
        unordered_map<string, int> reAllocCstmDm(const int total_dm_bw, Customer* cstm, double load_percent, bool is_new_over);
        void overDemands(vector<Site*> &vec_is_over_site, vector<Site*> &vec_can_over_site, const int total_dm_bw, int &curr_dm_bw, unordered_map<string, int> &slt_per_cstm, double load_percent);
        void balanceDemands(vector<Site*> &vec_usable_site, int &curr_dm_bw, unordered_map<string, int> &slt_per_cstm);
        void resetSite();
        void allocBwPerCstm(int bw, int &curr_dm_bw, Site* site, unordered_map<string, int> &slt_per_cstm);
